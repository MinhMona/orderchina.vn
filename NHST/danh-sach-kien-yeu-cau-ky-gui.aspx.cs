                        ...
                        if (item.SmallPackageID != null)
                        {
                            if (item.SmallPackageID > 0)
                            {
                                ...
                                if (pack != null)
                                {
                                    ...
                                    if (pack.Volume.ToString().ToFloat(0) > 0)
                                        volume = Convert.ToDouble(pack.Volume);
                                }
                            }
                        }
                        ltr.Text += "<td>" + weight + "</td>";
                        ltr.Text += "<td>" + volume + "</td>";
                        ltr.Text += "<td>" + item.IsInsurranceMoney + "</td>";
            ...
            if (HttpContext.Current.Session["userLoginSystem"] != null)
            {
                ...
                if (acc != null)
                {
                    ...
                    if (ts.Count > 0)
                    {
                        ...
                        double TotalInsurranceFeeCYN = 0;
                        double TotalInsurranceFeeVND = 0;
                        ...
                        double totalVolumePriceVND = 0;
                        double totalVolumePriceCYN = 0;
                        ...
                        foreach (var t in ts)
                        {
                            var checkwh = lw.Where(x => x.WareHouseID == t.WareHouseID && x.WareHouseFromID == t.WareHouseFromID && x.ShippingTypeID == t.ShippingTypeID).SingleOrDefault();
                            if (checkwh != null)
                            {
                                        ...
                                        if (package != null)
                                        {
                                            ...
                                            if (package.Weight != null)
                                            {
                                                if (package.Weight > 0)
                                                {
                                                    ...
                                                    totalWeight += weight;
                                                    p.Weight = weight;
                                                    volume = Convert.ToDouble(package.Volume);
                                                    totalVolume += volume;
                                                    p.Weight = weight;
                                                    p.Volume = volume;
                                                    p.TransportationID = t.ID;
                                                    checkwh.TotalWeight += weight;
                                                    checkwh.TotalVolume += volume;
                            ...
                            double sensorVND = 0;
                            double sensorCYN = 0;
                            double insurranceVND = 0;
                            double insurranceCYN = 0;
                            ...
                            if (!string.IsNullOrEmpty(t.IsInsurranceMoney))
                                if (t.IsInsurranceMoney.ToFloat(0) > 0)
                                    insurranceVND = Convert.ToDouble(t.IsInsurranceMoney);

                            insurranceCYN = insurranceVND / currency;
                            ...
                            TotalInsurranceFeeVND += insurranceVND;
                            TotalInsurranceFeeCYN += insurranceCYN;
                        }

                        if (acc.FeeTQVNPerWeight.ToFloat(0) > 0)
                        {
                            totalWeightPriceVND = Convert.ToDouble(acc.FeeTQVNPerWeight) * totalWeight;
                            totalWeightPriceCYN = Math.Round(totalWeightPriceVND / currency, 1);
                        }
                        if (acc.FeeTQVNPerVolume.ToFloat(0) > 0)
                        {
                            totalVolumePriceVND = Convert.ToDouble(acc.FeeTQVNPerVolume) * totalVolume;
                            totalVolumePriceCYN = Math.Round(totalVolumePriceVND / currency, 1);
                        }
                        else
                        {
                            if (lw.Count > 0)
                            {
                                ...
                                {
                                    var feeCN = WarehouseFeeController.GetByAndWarehouseFromAndToWarehouseAndShippingTypeAndAndHelpMoving(
                            item.WareHouseFromID, item.WareHouseID, item.ShippingTypeID, true);
                                    ...
                                    var feeTT = VolumeFeeController.GetByAndWarehouseFromAndToWarehouseAndShippingTypeAndAndHelpMoving(
                            item.WareHouseFromID, item.WareHouseID, item.ShippingTypeID, true);
                                    if (feeTT.Count > 0)
                                    {
                                        foreach (var f in feeTT)
                                        {
                                            totalVolumePriceVND = Convert.ToDouble(f.Price) * item.TotalVolume;
                                            totalVolumePriceCYN = Math.Round(totalVolumePriceVND / currency, 1);
                                            break;
                                        }
                                    }
                        ...
                        double FinalMoneyVND = 0;
                        double FinalMoneyCYN = 0;

                        if (totalWeightPriceVND > totalVolumePriceVND)
                        {
                            FinalMoneyVND = totalWeightPriceVND;
                            FinalMoneyCYN = totalWeightPriceCYN;
                        }
                        else
                        {
                            FinalMoneyVND = totalVolumePriceVND;
                            FinalMoneyCYN = totalVolumePriceCYN;
                        }

                        totalPriceVND = FinalMoneyVND + feeOutStockVND + TotalSensoredFeeVND + TotalAdditionFeeVND + TotalInsurranceFeeVND;
                        totalPriceCYN = FinalMoneyCYN + feeOutStockCYN + TotalSensoredFeeCYN + TotalSensoredFeeCYN + TotalInsurranceFeeCYN;
            ...
            if (acc != null)
            {
                ...
                if (!string.IsNullOrEmpty(strListID))
                {
                    ...
                    if (listID.Length - 1 > 0)
                    {
                        ...
                        double TotalFeeInsurranceVND = 0;
                        double TotalFeeInsurranceCYN = 0;
                        ...
                        for (int i = 0; i < listID.Length - 1; i++)
                        {
                            ...
                            if (t != null)
                            {
                                ...
                                TotalFeeInsurranceVND += insurrancevnd;
                                TotalFeeInsurranceCYN += insurrancecyn;
                            }
                        }
                        ...
                        if (acc.FeeTQVNPerVolume.ToFloat(0) > 0)
                        {
                            totalWeightPriceVND = Convert.ToDouble(acc.FeeTQVNPerVolume) * totalVolume;
                            totalVolumePriceCYN = Math.Round(totalVolumePriceVND / currency, 1);
                        }
                        else
                        {
                        ...
                        totalPriceVND = FinalMoneyVND + feeOutStockVND + TotalSensoredFeeVND + TotalAdditionFeeVND + TotalFeeInsurranceVND;
                        totalPriceCYN = FinalMoneyCYN + feeOutStockCYN + TotalSensoredFeeCYN + TotalSensoredFeeCYN + TotalFeeInsurranceCYN;
            ...
            if (acc != null)
            {
                ...
                if (!string.IsNullOrEmpty(strListID))
                {
                    ...
                    if (listID.Length - 1 > 0)
                    {
                        ...
                        int shippingtype = hdfShippingType.Value.ToInt(0);
                        int totalpackage = listID.Length - 1;
                        string kq = ExportRequestTurnController.InsertWithUIDVolume(UID, username, 0, currentDate, totalPriceVND,
                            totalPriceCYN, totalWeight, totalVolume, note, shippingtype, currentDate, username, totalpackage, 1);
