...
namespace NHST.Controllers
{
    public class PayHelpTempController
    {
        ...
        public static tbl_PayHelpTemp GetByID(int UID, int ID)
        {
            using (var db = new NHSTEntities())
            {
                var p = db.tbl_PayHelpTemp.Where(x => x.ID == ID && x.UID == UID).SingleOrDefault();
                if (p != null)
                    return p;
                else return null;
            }
        ...
        public static string Delete(int ID)
        {
            using (var db = new NHSTEntities())
            {
                var p = db.tbl_PayHelpTemp.Where(x => x.ID == ID).SingleOrDefault();
                if (p != null)
                {
        ...
        public static tbl_PayHelpTemp GetByOrderID(string OrderID)
        {
            using (var db = new NHSTEntities())
            {
                var ps = db.tbl_PayHelpTemp.Where(x => x.OrderID == OrderID).SingleOrDefault();
                if (ps != null)
                    return ps;
                return null;
            }
