            ...
            using (NHSTEntities dbe = new NHSTEntities())
            {
                ...
                using (var transaction = dbe.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        var account = dbe.tbl_Account.Where(ac => ac.ID == userID).SingleOrDefault();
                        if (account != null)
                        {
                        ...
                        var or = dbe.tbl_MainOder.Where(o => o.UID == userID && o.ID == orderID).SingleOrDefault();
                        if (or != null)
                        {
                            or.Status = status;
                        ...
                        //var or2 = dbe.tbl_MainOder.Where(o => o.ID == orderID).SingleOrDefault();
                        //if (or2 != null)
                        //{
            ...
            using (NHSTEntities dbe = new NHSTEntities())
            {
                ...
                using (var transaction = dbe.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        var a = dbe.tbl_Account.Where(ac => ac.ID == userID).SingleOrDefault();
                        if (a != null)
                        {
                            a.Wallet = walletLeft;
        ...
        public static int PaymentPayhelp(int PayhelpID, int uid, string userName, double walletleft, double walletCYN_left, double amount, string content, int typeVND, int tradeTypeVND, int type, int tradeType, double TotalPrice, int status, DateTime currentDate)
        {
            using (NHSTEntities dbe = new NHSTEntities())
            {
                ...
                using (var transaction = dbe.Database.BeginTransaction())
                {
                    try
                    {
                        var acc = dbe.tbl_Account.Where(ac => ac.ID == uid).SingleOrDefault();
                        if (acc != null)
                        {
           ...
           int type, int typeTrade)
        {
            using (NHSTEntities dbe = new NHSTEntities())
            {
                ...
                using (var transaction = dbe.Database.BeginTransaction())
                {
                    try
                    {
                        var p = dbe.tbl_TransportationOrder.Where(pa => pa.ID == transOrderID).SingleOrDefault();
                        if (p != null)
                        {
                        ...
                        var a = dbe.tbl_Account.Where(ac => ac.ID == uid).SingleOrDefault();
                        if (a != null)
                        {
                            a.Wallet = walletLeft;
                            a.ModifiedBy = username;
