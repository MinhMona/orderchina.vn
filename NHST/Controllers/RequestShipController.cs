...
namespace NHST.Controllers
{
    public class RequestShipController
    {
            ...
            DateTime ModifiedDate, string ModifiedBy)
        {
            using (var dbe = new NHSTEntities())
            {
                var p = dbe.tbl_RequestShip.Where(pa => pa.ID == ID).SingleOrDefault();
                if (p != null)
                {
                    p.ListOrderCode = ListOrderCode;
                    p.Note = Note;
        ...
        public static tbl_RequestShip GetByID(int ID)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_RequestShip page = dbe.tbl_RequestShip.Where(p => p.ID == ID).SingleOrDefault();
                if (page != null)
                    return page;
                else
                    return null;
        ...
        public static tbl_RequestShip GetByUIDAndID(int UID, int ID)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_RequestShip page = dbe.tbl_RequestShip.Where(p => p.UID == UID && p.ID == ID).SingleOrDefault();
                if (page != null)
                    return page;
                else
                    return null;
