...
namespace NHST.Controllers
{
    public class AccountController
    {
        ...
        public static tbl_Account Login(string Username, string Password)
        {
            using (var dbe = new NHSTEntities())
            {
                var ac = dbe.tbl_Account.Where(x => x.Username == Username).SingleOrDefault();
                if (ac != null)
                {
        ...
        public static string UpdateFee(int ID, string FeeBuyPro, string FeeTQVNPerWeight, string FeeTQVNPerVolume)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_Account.Where(ac => ac.ID == ID).SingleOrDefault();
                if (a != null)
                {
                    a.FeeBuyPro = FeeBuyPro;
                    a.FeeTQVNPerWeight = FeeTQVNPerWeight;
                    a.FeeTQVNPerVolume = FeeTQVNPerVolume;
                    dbe.Configuration.ValidateOnSaveEnabled = false;
                    string kq = dbe.SaveChanges().ToString();
                    return kq;
                }
        ...
        public static string DeleteStaff(int ID)
        {
            using (var db = new NHSTEntities())
            {
                var acc = db.tbl_Account.Where(o => o.ID == ID && o.RoleID != 1).SingleOrDefault();
                if (acc != null)
                {
                    db.tbl_Account.Remove(acc);
                    db.Configuration.ValidateOnSaveEnabled = false;
        ...
        public static tbl_Account GetByIDCustomeOrtherUID(int UID, string IDCustomer)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_Account acc = dbe.tbl_Account.Where(a => a.ID != UID && a.MaKH == IDCustomer).FirstOrDefault();
                if (acc != null)
                    return acc;
                else
                    return null;
        ...
        public static string InsertMaKH(int ID, string idcustomer, string CreatedBy, DateTime CreatedDate)
        {
            using (var dbe = new NHSTEntities())
            {
        ...
        public static string UpdateMaKH(int ID, string IDCustomer, string ModifiedBy, DateTime ModifiedDate)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_Account.Where(ac => ac.ID == ID).FirstOrDefault();
        ...
        public static int GetTotalUser_searchid_searchsdt(string searchName, string searchid, string searchsdt)
        {
            ...
            if (!string.IsNullOrEmpty(searchid))
                sql += "and MaKH Like N'%" + searchid + "%' ";
            if (!string.IsNullOrEmpty(searchsdt))
                sql += "and MobilePhone Like N'%" + searchsdt + "%' ";
            var reader = (IDataReader)SqlHelper.ExecuteDataReader(sql);
            int a = 0;
            while (reader.Read())
            {
        ...
        public static List<View_UserList> GetListUserBySQL_searchid_searchsdt(string searchname, string searchid, string searchsdt, int pageIndex, int pageSize)
        {
            var sql = @"select* ";
            sql += "from View_UserList ";
                ...
                using (var transaction = dbe.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (ac != null)
                        {
                            ...
                            var a = dbe.tbl_Account.Where(x => x.ID == ac.ID).SingleOrDefault();
                            if (a != null)
                            {
                                a.Wallet = walletleft;
                            }
                            ...
                            var checkhis = dbe.tbl_HistoryAutoBanking.Where(x => x.UID == UID && x.Note == Note).SingleOrDefault();
                            if (checkhis != null)
                            {
                                checkhis.Status = 2;
                            }
