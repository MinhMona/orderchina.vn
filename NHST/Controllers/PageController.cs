...
namespace NHST.Controllers
{
    public class PageController
    {
        ...
        public static string Update(int ID, string Title, string Summary, string IMG, string PageContent, bool IsHidden, int PageTypeID,
            ...
            string ogFBIMG, string ogTWtitle, string ogTWDescription, string ogTWIMG, bool SideBar, int IndexPos)
        {
            using (var dbe = new NHSTEntities())
            {
        ...
        public static List<tbl_Page> GetAllByIndexPos()
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_Page> pages = new List<tbl_Page>();
                pages = dbe.tbl_Page.OrderBy(p => p.IndexPos).ToList();
                if (pages.Count > 0)
                {
                    return pages;
                }
        ...
        public static List<tbl_Page> GetByPagetTypeID(int PageTypeID)
        {
            ...
            {
                List<tbl_Page> pages = new List<tbl_Page>();
                pages = dbe.tbl_Page.Where(p => p.PageTypeID == PageTypeID && p.IsHidden == false).OrderBy(p => p.IndexPos).ToList();
                return pages;
            }
        }
        ...
        public static string Delete(int ID)
        {
            using (var dbe = new NHSTEntities())
            {
                var tw = dbe.tbl_Page.Where(o => o.ID == ID).SingleOrDefault();
                if (tw != null)
                {
                    dbe.tbl_Page.Remove(tw);
                    dbe.Configuration.ValidateOnSaveEnabled = false;
