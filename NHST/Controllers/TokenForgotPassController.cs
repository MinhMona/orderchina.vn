...
namespace NHST.Controllers
{
    public class TokenForgotPassController
    {
        ...
        public static string Update(int ID)
        {
            using (var db = new NHSTEntities())
            {
                var tk = db.tbl_TokenForgotPass.Where(x => x.ID == ID).SingleOrDefault();
                if (tk != null)
                {
                    tk.IsHidden = true;
        ...
        public static tbl_TokenForgotPass GetByToken(string Token)
        {
            using (var db = new NHSTEntities())
            {
                var tk = db.tbl_TokenForgotPass.Where(x => x.Token == Token && x.IsHidden == false).SingleOrDefault();
                if (tk != null)
                    return tk;
                else
                    return null;
        ...
        public static tbl_TokenForgotPass GetToken(string Token)
        {
            using (var db = new NHSTEntities())
            {
                var tk = db.tbl_TokenForgotPass.Where(x => x.Token == Token).SingleOrDefault();
                if (tk != null)
                    return tk;
                else
                    return null;
