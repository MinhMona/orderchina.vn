...
namespace NHST.Controllers
{
    ...
    {        
        ...
        public static string UpdateStatusByID(int ID, bool Status)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_OTP o = dbe.tbl_OTP.Where(ot => ot.ID == ID).SingleOrDefault();
                if (o != null)
                {
                    o.Status = Status;
                    dbe.Configuration.ValidateOnSaveEnabled = false;
        ...
        public static tbl_OTP SelectByUserPhonePrefixTypeValue(int UID, string UserPhone, string Prefix, string Value, int Type)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_OTP otp = dbe.tbl_OTP.Where(ot => ot.UserPhone == UserPhone && ot.Prefix == Prefix && ot.Type == Type && ot.Value == Value).SingleOrDefault();
                if (otp != null)
                {
                    return otp;
                }
