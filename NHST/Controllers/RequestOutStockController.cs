...
namespace NHST.Controllers
{
    public class RequestOutStockController
    {
        ...
        public static List<RequestOutStoc> GetAllSQLHelper1(string s)
        {
            ...
            var sql = @"SELECT ro.*, sp.OrderTransactionCode, sp.Weight, sp.Volume ,sp.DateInLasteWareHouse, sp.DateOutWarehouse, er.Note, er.ShippingTypeVNName ";
            ...
            while (reader.Read())
            {
                ...
                {
                    if (reader["Volume"].ToString().ToFloat(0) > 0)
                    {
                        volume = Convert.ToDouble(reader["Volume"]);
                    }
                }
                entity.Weight = weight.ToString();
                entity.Volume = volume.ToString();
                if (reader["MainOrderID"] != DBNull.Value)
