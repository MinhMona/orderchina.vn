...
namespace NHST.Controllers
{
    public class FeeCheckProductController
    {
        ...
        public static string Update(int ID, double AmountFrom, double AmountTo, double Price, int Type, DateTime ModifiedDate, string ModifiedBy)
        {
            ...
            {
                dbe.Configuration.ValidateOnSaveEnabled = false;
                var a = dbe.tbl_FeeCheckProduct.Where(f => f.ID == ID).SingleOrDefault();
                if (a != null)
                {
                    a.AmountFrom = AmountFrom;
                    a.AmountTo = AmountTo;
        ...
        public static tbl_FeeCheckProduct GetByWeightAndRecivePlaceAndAmount(double WeightFrom, double WeightTo, double Amount, int Type)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_FeeCheckProduct.Where(f => f.AmountFrom == WeightFrom && f.AmountTo == WeightTo && f.Price == Amount && f.Type == Type).SingleOrDefault();
                if (a != null)
                {
                    return a;
                }
        ...
        public static tbl_FeeCheckProduct GetByPriceFromAndPriceTo(double PriceFrom, double PriceTo)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_FeeCheckProduct.Where(f => f.AmountFrom == PriceFrom && f.AmountTo == PriceTo).SingleOrDefault();
                if (a != null)
                {
                    return a;
                }
        ...
        public static tbl_FeeCheckProduct GetByBetweenAmountAndType(double Amount, int Type)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_FeeCheckProduct.Where(f => f.AmountFrom <= Amount && f.AmountTo >= Amount && f.Type == Type).SingleOrDefault();
                if (a != null)
                {
                    return a;
                }
        ...
        public static tbl_FeeCheckProduct GetByQuantityAndType(int Quantity, int Type)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_FeeCheckProduct.Where(f => f.AmountFrom >= Quantity && f.AmountTo < Quantity && f.Type == Type).SingleOrDefault();
                if (a != null)
                {
                    return a;
                }
