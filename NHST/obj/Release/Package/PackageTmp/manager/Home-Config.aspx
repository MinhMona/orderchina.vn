...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <asp:Button runat="server" ID="btnDeleteStep" Style="display: none" OnClick="btnDeleteStep_Click" />
    <asp:Button runat="server" ID="btnDeleteService" Style="display: none" OnClick="btnDeleteService_Click" />
    <asp:Button runat="server" ID="btnDeleteBenefits" Style="display: none" OnClick="btnDeleteBenefits_Click" />
    <script type="text/javascript">
        ...
        function DeleteStep(ID) {
            var c = confirm('Bạn muốn xóa bước này??');
            if (c) {
                $("#<%=hdfStepID.ClientID%>").val(ID);
                $("#<%=btnDeleteStep.ClientID%>").click();
            }
        }

        function DeleteService(ID) {
            var c = confirm('Bạn muốn xóa dịch vụ này??');
            if (c) {
                $("#<%=hdfEditServicesID.ClientID%>").val(ID);
                $("#<%=btnDeleteService.ClientID%>").click();
            }
        }

        function DeleteBenefits(ID) {
            var c = confirm('Bạn muốn xóa quyền lợi này??');
            if (c) {
                $("#<%=hdfEditBenefitID.ClientID%>").val(ID);
                $("#<%=btnDeleteBenefits.ClientID%>").click();
            }
        }
