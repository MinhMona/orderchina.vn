    ...
    public partial class Home_Config : System.Web.UI.Page
    {
        ...
        public void LoadStep()
        {
            ...
            for (int i = 0; i < listStep.Count; i++)
            {
                ...
                hcm.Append("<a href=\"javascript:;\" id=\"DeleteStep-" + item.ID + "\" onclick=\"DeleteStep(" + item.ID + ")\" class=\" modal-trigger\" data-position=\"top\"><i class=\"material-icons\">delete</i><span>Xóa</span></a>");
                hcm.Append("</div>");
                hcm.Append("</td>");
                hcm.Append("</tr>");
            }
        ...
        private void LoadService()
        {
            ...
            for (int i = 0; i < listService.Count; i++)
            {
                ...
                hcm.Append("<a href=\"javascript:;\" id=\"DeleteService-" + item.ID + "\" onclick=\"DeleteService(" + item.ID + ")\" class=\" modal-trigger\" data-position=\"top\"><i class=\"material-icons\">delete</i><span>Xóa</span></a>");
                hcm.Append("</div>");
                hcm.Append("</td>");
                hcm.Append("</tr>");
            }
        ...
        private void LoadCustomersBenefit()
        {
            ...
            for (int i = 0; i < customerBenefits.Count; i++)
            {
                ...
                hcm.Append("<a href=\"javascript:;\" id=\"DeleteBenefits-" + item.ID + "\" onclick=\"DeleteBenefits(" + item.ID + ")\" class=\" modal-trigger\" data-position=\"top\"><i class=\"material-icons\">delete</i><span>Xóa</span></a>");
                hcm.Append("</div>");
                hcm.Append("</td>");
                hcm.Append("</tr>");
            }
        ...
        protected void btnDeleteStep_Click(object sender, EventArgs e)
        {
            string Email = Session["userLoginSystem"].ToString();
            DateTime currentDate = DateTime.Now;
            int CBID = hdfStepID.Value.ToInt(0);

            var menu = StepController.GetByID(CBID);
            if (menu != null)
            {
                StepController.Delete(menu.ID);
                PJUtils.ShowMessageBoxSwAlert("Xóa bước thành công.", "s", true, Page);
            }
        }
        ...
        protected void btnDeleteService_Click(object sender, EventArgs e)
        {
            ...
            var menu = ServiceController.GetByID(CBID);
            if (menu != null)
            {
                ServiceController.Delete(menu.ID);
                PJUtils.ShowMessageBoxSwAlert("Xóa dịch vụ thành công.", "s", true, Page);
            }
        }

        protected void btnDeleteBenefits_Click(object sender, EventArgs e)
        {
            string Email = Session["userLoginSystem"].ToString();
            DateTime currentDate = DateTime.Now;
            int CBID = hdfEditBenefitID.Value.ToInt(0);

            var menu = CustomerBenefitsController.GetByID(CBID);
            if (menu != null)
            {
                CustomerBenefitsController.Delete(menu.ID);
                PJUtils.ShowMessageBoxSwAlert("Xóa quyền lợi thành công.", "s", true, Page);
            }
        }
    }
}