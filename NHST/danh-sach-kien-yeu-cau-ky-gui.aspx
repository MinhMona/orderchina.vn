                    ...
                    success: function (msg) {
                        ...
                        if (ret != "0") {
                            ...
                            if (c == true) {
                                ...
                                var totalVolumePriceCYN = data[13];
                                var totalVolumePriceVND = data[14];
                                var totalVolumePriceVNDstr = formatThousands(totalVolumePriceVND, 0);
                                var FinalMoneyCYN = data[15];
                                var FinalMoneyVND = data[16];
                                var FinalMoneyVNDstr = formatThousands(FinalMoneyVND, 0);
                                var totalVolume = data[17];
                                var FeeInsurranceVND = data[18];
                                var FeeInsurranceVNDstr = formatThousands(FeeInsurranceVND, 0);
                                if (status == 1) {
                                    ...
                                    html += "</div>";
                                    html += "<div class=\"popup-row\">";
                                    if (totalWeightPriceVNDstr > totalVolumePriceVNDstr) {
                                        html += "<p>Tổng số kg xuất kho: " + totalWeight
                                            + " kg - <strong>" + FinalMoneyVNDstr + "</strong> VNĐ.</p>";
                                    }
                                    else {
                                        html += "<p>Tổng m3 thể tích xuất kho: " + totalVolume
                                            + " m3 - <strong>" + FinalMoneyVNDstr + "</strong> VNĐ.</p>";
                                    }
                                    html += "</div>";
                                    html += "<div class=\"popup-row\">";
                                    html += "<p>Phí bảo hiểm: <strong>" + FeeInsurranceVNDstr + "</strong> VNĐ.</p>";
                                    html += "</div>";
                                    html += "<div class=\"popup-row\">";
                                    ...
                                    $("#<%=hdfListID.ClientID%>").val(listID);
                                    var leftmoney = data[18];
                                    var leftmoneystr = formatThousands(leftmoney, 0);
                                    var FeeInsurranceVND = data[19];
                                    var FeeInsurranceVNDstr = formatThousands(FeeInsurranceVND, 0);
                                    var button = "";
                                    button += "             <a href=\"javascript:;\" class=\"btn btn-close pill-btn primary-btn main-btn hover\" onclick='ThanhToanTaiKho($(this))'>Thanh toán tại kho</a>";
                                    var html = "";
                                    ...
                                    html += "</div>";
                                    html += "<div class=\"popup-row\">";
                                    html += "<span class=\"popuperror\" style=\"color:red\">Để nạp thêm tiền vào tài khoản, vui lòng truy cập <a href=\"/nap-tien\" target=\"_blank\"><strong style=\"font-size:20px;\">TẠI ĐÂY</strong></a>.</span>";
                                    html += "</div>";
                                    showPopup("Thanh toán xuất kho", html, button);
                                }
                            }
                ...
                success: function (msg) {
                    ...
                    if (ret != "0") {
                        ...
                        if (c == true) {
                            ...
                            var totalAdditionFeeVNDstr = formatThousands(totalAdditionFeeVND, 0);
                            var TotalSensoredFeeVND = data[14];
                            var TotalSensoredFeeVND = data[15];
                            var TotalSensoredFeeVNDstr = formatThousands(TotalSensoredFeeVND, 0);
                            var totalVolumePriceCYN = data[16];
                            var totalVolumePriceVND = data[17];
                            var totalVolumePriceVNDstr = formatThousands(totalVolumePriceVND, 0);
                            var FinalMoneyCYN = data[18];
                            var FinalMoneyVND = data[19];
                            var FinalMoneyVNDstr = formatThousands(FinalMoneyVND, 0);
                            var totalVolume = data[20];
                            var TotalInsurranceFeeVND = data[21];
                            var TotalInsurranceFeeVNDstr = formatThousands(TotalInsurranceFeeVND, 0);
                            if (status == 1) {
                                $("#<%=hdfListID.ClientID%>").val(listID);
                                var button = "<a href=\"javascript:;\" class=\"btn btn-close pill-btn primary-btn main-btn hover\" onclick=\"paytoexport($(this))\">Thanh toán</a>";
                                button += "             <a href=\"javascript:;\" class=\"btn btn-close pill-btn primary-btn main-btn hover\" onclick='ThanhToanTaiKho($(this))'>Thanh toán tại kho</a>";
                                ...
                                html += "</div>";
                                html += "<div class=\"popup-row\">";
                                html += "<p>Phí bảo hiểm: <strong>" + TotalInsurranceFeeVNDstr + "</strong> VNĐ.</p>";
                                html += "</div>";
                                html += "<div class=\"popup-row\">";
                                html += "<p>Phí hải quan xuất kho cố định: <strong>" + feeOutStockVNDstr + "</strong> VNĐ.</p>";
                                html += "</div>";
                                ...
                                $("#<%=hdfListID.ClientID%>").val(listID);
                                var leftmoney = data[21];
                                var leftmoneystr = formatThousands(leftmoney, 0);
                                var TotalInsurranceFeeVND = data[22];
                                var TotalInsurranceFeeVNDstr = formatThousands(TotalInsurranceFeeVND, 0);
                                var button = "";
                                button += "             <a href=\"javascript:;\" class=\"btn btn-close pill-btn primary-btn main-btn hover\" onclick='ThanhToanTaiKho($(this))'>Thanh toán tại kho</a>";
                                var html = "";
